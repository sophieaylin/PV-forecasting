    ...
    for filename in filenames:
        ...
        for dataset, group in df.groupby("dataset"):
            for model in [baseline, "ols", "ridge", "lasso"]:
                ...
                # error metrics [W/m^2]
                error = (group["{}_{}".format(target,horizon)].values - group["{}_{}".format(target, model)].values)
                ...
                # and reference baseline model, respectively. (smart persistence model)
                rmse_p = np.sqrt(
...
def summary_table(target):
    ...
    for model, group in df.groupby(["model"]):
        ...
        newrow = {"model": model, "MAE": group.mean()["MAE"] , "st":group.std()["MAE"]
                     ...
                     , "skill": group.mean()["skill"] * 100, "st":group.std()["skill"] * 100}
